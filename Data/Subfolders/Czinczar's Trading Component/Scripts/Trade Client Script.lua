...
function OnProposeOffer(whichButton)
	...
	for _, line in pairs(yourOffer_UIScrollPanel:GetChildren()) do
		...
		local itemToCheck = ItemDatabase:GetItemFromName(resourceName)
...
function DisplayResources()
	...
	local slot = localInventory:GetAllItems()

	for slotIndex, item in localInventory:IterateBackpackSlots() do 
	print(slotIndex)
	print(item)
	end
	local t = {}
	for _, itemSlot in pairs(slot) do
		k = itemSlot.item:GetName()
		--print(k)
		local itemToCheck = ItemDatabase:GetItemFromName(k)
		local amount = localInventory:GetItemStackSum(itemToCheck)
		--print(amount)
		t[k] = amount
	end
