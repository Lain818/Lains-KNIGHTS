...
local function OnRewardCollected(player, reward)
	if(reward.type ~= DailyRewardsAPI.RewardType.RESOURCE_PEARL_RT) then return end
...
print(playerData[DATA_KEY])
	...
	if(#RESOURCE_NAME > 0) then
        ...
        local item = ItemDatabase:GetItemFromName(RESOURCE_NAME)
